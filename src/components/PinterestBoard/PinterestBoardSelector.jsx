...
const PinterestBoardSelector = ({onSelectionChange}) => {
  ...
  return (<div className="pinterest-selector">
    <div className="selector-type">
      <h4>Select Board:</h4>

      <div className="board-selector">
        <label htmlFor="pinterest-board">Choose a board:</label>
        <select id="pinterest-board" value={selectedBoardId || ""} onChange={handleBoardChange} disabled={loading || boards.length === 0}>
        ...
        {!accessToken && (<p className="error">Please connect your Pinterest account first</p>)}
      </div>
    </div>
  </div>);
};
